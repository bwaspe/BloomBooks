...
// EPX is set to "all taxable", so its tax is 8.375% of the sale by definition
// and dsDayTax works it out at report time. There was a button here that wrote
// that same figure onto each day, and a function behind it; both are gone.
// Writing an arithmetic result down and then comparing it against the same
// arithmetic proves nothing, and the Sales Tax page had ended up telling the
// owner "nothing needs entering" beside a prominent button offering to enter
// it. Pressing it moved $3,083 from "worked out" to "checked" and changed no
// figure at all.
//
// A tax typed on a day by hand still wins over the derived one -- that is
// dsDayTax's first rule -- so a genuine correction is still possible.
function epxSetDayBook(dateIso, amount) {
  ...
  // No tax written. Every counter card sale is taxable, so the tax is the rate
  // on the sale and dsDayTax derives it; recording it here as well would only
  // decide, arbitrarily, which days count as "checked" on the sales-tax page
  // depending on which ones happened to be set from a statement.
  rec[EPX_CHANNEL] = { s: val, x: val };
  saveData();
